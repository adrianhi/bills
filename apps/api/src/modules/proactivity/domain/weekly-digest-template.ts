...
export function renderWeeklyDigestHtml(input: WeeklyDigestTemplateInput): {
  ...
  const name = escapeHtml(input.userDisplayName || 'ahorrador');
  const appUrl = input.appUrl || 'http://localhost:3000';
  const footer = emailFooter(input.unsubscribeUrl, appUrl);
  ...
  const billsRows = input.upcomingBills.length > 0
    ? input.upcomingBills
        ...
        <tr style="border-bottom: 1px solid #1e293b;">
          <td style="padding: 10px 0; font-size: 13px; color: #f1f5f9; font-weight: 600;">${escapeHtml(b.displayName)}</td>
          <td style="padding: 10px 0; font-size: 12px; color: #94a3b8; text-align: center;">${escapeHtml(b.nextExpectedDate || 'Próximamente')}</td>
          <td style="padding: 10px 0; font-size: 13px; color: #10b981; font-weight: 700; text-align: right;">${formatEmailMoney(b.expectedAmount, b.currency)}</td>
        </tr>`
        )
        .join('')
  ...
  const topCategoryHtml = c.topCategory
    ? `<p style="margin: 4px 0 0 0; font-size: 13px; color: #cbd5e1;">Categoría con más gasto: <strong style="color: #f1f5f9;">${escapeHtml(c.topCategory.name)}</strong> (${formatEmailMoney(c.topCategory.amount, currency)} · ${c.topCategory.percentage}%)</p>`
    : '';

  const topMerchantHtml = c.topMerchant
    ? `<p style="margin: 4px 0 0 0; font-size: 13px; color: #cbd5e1;">Comercio frecuente: <strong style="color: #f1f5f9;">${escapeHtml(c.topMerchant.name)}</strong> (${formatEmailMoney(c.topMerchant.amount, currency)})</p>`
    : '';
...
<body style="margin: 0; padding: 24px 12px; background-color: #030712; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #f1f5f9;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 580px; margin: 0 auto; background-color: #0b1329; border-radius: 20px; border: 1px solid #1e293b; overflow: hidden;">
    ...
    <tr>
      <td style="padding: 28px 24px 20px 24px; text-align: center; border-bottom: 1px solid #1e293b; background: linear-gradient(180deg, #111e38 0%, #0b1329 100%);">
        <div style="display: inline-block; background-color: #10b981; color: #ffffff; font-weight: 900; font-size: 18px; width: 36px; height: 36px; line-height: 36px; border-radius: 10px; margin-bottom: 8px;">C.</div>
    ...
    <tr>
      <td style="padding: 24px;">
        <p style="margin: 0 0 18px 0; font-size: 14px; color: #cbd5e1;">Hola <strong>${name}</strong>, aquí tienes el resumen de tus últimos siete días:</p>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #131f37; border-radius: 14px; border: 1px solid #1e293b; margin-bottom: 16px;">
          <tr>
            <td style="padding: 16px;">
              ...
              <div style="font-size: 26px; font-weight: 900; color: #ffffff; margin: 6px 0 4px 0;">${formatEmailMoney(c.totalSpentThisWeek, currency)}</div>
          ...
          <tr>
            <td style="padding: 16px;">
              ...
              <div style="margin-top: 6px; font-size: 13px; color: #f1f5f9;">
                ...
                <div style="font-size: 20px; font-weight: 900; color: #10b981; margin-top: 2px;">${formatEmailMoney(c.estimatedDailyAllowance, currency)} / día</div>
              </div>
            </td>
          </tr>
        </table>
        ...
        <div style="text-align: center; margin-bottom: 12px;">
          <a href="${escapeHtml(appUrl)}" style="display: inline-block; background-color: #10b981; color: #ffffff; font-weight: 700; font-size: 14px; text-decoration: none; padding: 12px 28px; border-radius: 12px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.25);">Abrir Cuadre</a>
        </div>
      </td>
    </tr>
  ...
  const text = [
    `Tu Pulso Semanal (${c.startDate} al ${c.endDate})`,
    `Hola ${input.userDisplayName || 'ahorrador'},`,
    `Gasto de los últimos 7 días: ${formatEmailMoney(c.totalSpentThisWeek, currency)}.`,
    c.changePercent === null ? 'Sin comparación disponible.' : `Cambio frente a los 7 días anteriores: ${c.changePercent}%.`,
    c.topCategory ? `Categoría principal: ${c.topCategory.name} (${formatEmailMoney(c.topCategory.amount, currency)}).` : '',
    c.topMerchant ? `Comercio principal: ${c.topMerchant.name} (${formatEmailMoney(c.topMerchant.amount, currency)}).` : '',
    ...input.upcomingBills.slice(0, 4).map((bill) => `${bill.displayName}: ${formatEmailMoney(bill.expectedAmount, bill.currency)} — ${bill.nextExpectedDate}.`),
    `Margen recomendado: ${formatEmailMoney(c.estimatedDailyAllowance, currency)} por día.`,
    `Abrir Cuadre: ${appUrl}`,
    footer.text,
  ].filter(Boolean).join('\n\n');
