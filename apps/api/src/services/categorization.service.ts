...
export class CategorizationService {
  private static builtInRules: BuiltInRule[] = [
    // Supermercados
  ...
  /**
   * Cleans raw merchant text (removes POS codes, special characters, extra locations)
   */
  public static cleanRawMerchant(raw: string): string {
    if (!raw) return 'Comercio Desconocido';
    let cleaned = raw.trim();

    // Remove common prefixes
    cleaned = cleaned.replace(/^(SM|EST|FARM|REST|TIENDA|SERV|COMPRA|PAGO)\s+/i, '');
  ...
  public static async categorize(
    ...
    if (explicitCategory && explicitCategory.trim() !== '' && explicitCategory !== 'Otros' && explicitCategory !== 'Sin Categoría') {
      return {
        merchant: explicitMerchant || this.cleanRawMerchant(rawMerchant),
        category: explicitCategory.trim(),
      };
    }
    ...
    for (const rule of this.builtInRules) {
      if (rule.pattern.test(textToMatch)) {
        return {
          merchant: explicitMerchant || rule.merchant,
    ...
    return {
      merchant: explicitMerchant || this.cleanRawMerchant(rawMerchant),
      category: explicitCategory || 'Otros',
    };
  }
}
