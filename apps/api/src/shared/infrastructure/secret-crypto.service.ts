...
function getKey(): Buffer {
  ...
  if (key.length !== 32) {
    throw new AppError(503, 'ENCRYPTION_NOT_CONFIGURED', 'A valid 32-byte INGESTION_ENCRYPTION_KEY is required.');
  }
  return key;
}
...
export class SecretCryptoService {
  ...
  public static decrypt(payload: string): string {
    ...
    const decipher = crypto.createDecipheriv('aes-256-gcm', getKey(), Buffer.from(encodedIv, 'base64url'));
