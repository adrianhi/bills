...
export function useBudgetManager(input: {
  ...
  const suggestionMutation = useMutation({
    ...
    onSuccess: (suggestion) => {
      setGlobalLimit(suggestion.globalLimit ? formatAmountInput(suggestion.globalLimit) : '');
      setLimits(Object.fromEntries(suggestion.categories.map((item) => [item.categoryKey, formatAmountInput(item.amount)])));
  ...
  const categoryTotal = Object.values(limits).reduce((sum, value) => sum + (Number(parseAmountInput(value)) || 0), 0);
  const globalValue = Number(parseAmountInput(globalLimit)) || 0;

  const save = async () => {
    const rawGlobal = parseAmountInput(globalLimit);
    const categories = Object.entries(limits)
      .filter(([, value]) => value.trim())
      .map(([categoryKey, value]) => ({ categoryKey, amount: Number(parseAmountInput(value)) }));

    if (rawGlobal && (!Number.isFinite(Number(rawGlobal)) || Number(rawGlobal) <= 0)) {
    ...
    await saveMutation.mutateAsync({
      ...
      globalLimit: rawGlobal ? Number(rawGlobal) : null, categories,
    });
  };
